...
assert.equal(map.remove("hat"), true);
assert.equal(map.keys().length, 11);
assert.equal(map.values().length, 11);
assert.equal(map.entries().length, 11);
assert.equal(map.length(), 11);
assert.equal(map.remove("car"), false);
map.clear();
